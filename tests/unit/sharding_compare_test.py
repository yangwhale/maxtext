...
class TestGetAbstractState:
  ...
  def test_get_abstract_state_sharding(self, abstract_state_and_shardings):  # pylint: disable=redefined-outer-name
    ...
    if hasattr(abstract_state, "params"):  # Linen TrainState
      ...
      # Filter to floating-point leaves only: abstract_state.model also contains
      # RNG state variables (uint32 / key dtype) which are not weight parameters.
      float_leaves = [
          l
          for l in jax.tree_util.tree_leaves(abstract_state.model)
          if isinstance(l, jax.ShapeDtypeStruct) and jnp.issubdtype(l.dtype, jnp.floating)
      ]
      assert len(float_leaves) > 0
      assert all(l.dtype == jnp.float32 for l in float_leaves)
